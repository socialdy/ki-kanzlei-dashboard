...
import {
  X, Trash2, ChevronDown, CheckSquare, Sparkles, Loader2,
} from "lucide-react";
...
const STATUS_OPTIONS: { value: SeoPostStatus; label: string; dot: string }[] = [
  { value: "draft",     label: SEO_STATUS_CONFIG.draft.label,     dot: SEO_STATUS_CONFIG.draft.dot },
  { value: "published", label: SEO_STATUS_CONFIG.published.label, dot: SEO_STATUS_CONFIG.published.dot },
];

interface SeoSelectionBarProps {
  selectedCount: number;
  totalCount?: number;
  onClear: () => void;
  onDelete: () => void;
  onStatusChange: (status: SeoPostStatus) => Promise<void>;
  onSelectAll?: () => void;
}

export function SeoSelectionBar({
  selectedCount,
  totalCount,
  onClear,
  onDelete,
  onStatusChange,
  onSelectAll,
}: SeoSelectionBarProps) {
  const [changingStatus, setChangingStatus] = useState(false);

  if (selectedCount === 0) return null;

  async function handleStatusChange(status: SeoPostStatus) {
